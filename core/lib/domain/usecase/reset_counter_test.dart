...
void main() {
  test('Reset counter test', () async {
    ...
    final SetCounterUseCase setCounter = SetCounterUseCaseImpl(repository);
    final ResetCounterUseCase usecase = ResetCounterUseCaseImpl(repository);

    final counter = await getCounter.execute();

    expect(counter.value, 0);

    await setCounter.execute(5);

    final tempCounter = await getCounter.execute();

    expect(tempCounter.value, 5);

    await usecase.execute();
    final newCounter = await getCounter.execute();

    expect(newCounter.value, 0);
  });
}
